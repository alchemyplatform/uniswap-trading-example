...
import { ChainId, Token, Fetcher, Route, Pair, Trade, TokenAmount, TradeType } from '@uniswap/sdk'
...
export class UniswapTradingPairsGraph {
  ...
  public async createTradingGraph() : Promise<void> {
    const allPairsLength = +(await this.uniswapFactory.methods
      .allPairsLength()
      .call());
    console.log(`There are ${allPairsLength} tradable pairs, but we will only get the first 1000 for the sake of time. Can you cache the results to speed things up next time?`);
    ...
    allPairs.forEach((pair) => {
      if (pair) {
        this.uniswapTradingGraph.setEdge(getAddress(pair.token0), getAddress(pair.token1), getAddress(pair.tokenPairAddress));
      }
    })
  ...
  public async getCheapestTradingPath(tokenAddress0: string, tokenAmount0: number, tokenAddress1: string): Promise<void> {
      ...
      if (path.length === new Set(path).size && path.length > 1) {
        ...
        for (const tokenAddr of path) {
          if (tokenAddrToToken[tokenAddr]) {
            ...
            const tokenObj = new Token(ChainId.MAINNET, tokenAddr, Number(await erc20Token.methods.decimals().call()));
        ...
        for (const [tokenFrom, tokenTo] of zipAdjacent(tokens)) {
          if (tokenFrom && tokenTo) {
            const tokenPairString = tokenFrom + "," + tokenTo;
        ...
        try {
          const route = new Route(tokenPairs, tokens[0], tokens[tokens.length - 1]);
          const trade = new Trade(route, new TokenAmount(tokens[0], BigInt(tokenAmount0)), TradeType.EXACT_INPUT);
          ...
          tokenAmount1ToPath[(Number(trade.executionPrice.toSignificant(10)) * tokenAmount0)] = path;
        } catch (err) {
          if (err.name == "InsufficientInputAmountError") {
            console.log(`Insufficient input amount to take path: ${path.join(" -> ")}`);
          } else {
            console.log(err);
          }
        }
    ...
    if (Object.entries(tokenAmount1ToPath).length != 0) {
      let maxValue = 0;
      console.log(tokenAmount1ToPath);
      ...
      console.log(`Optimal trading path: ${tokenAmount1ToPath[maxValue].join(" -> ")} : ${maxValue}`);
  ...
  private allPaths(tokenAddress0: string, tokenAddress1: string, visited : Set<string>, path: string[], length: number): void {
    // Full path found
